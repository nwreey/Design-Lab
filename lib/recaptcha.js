...
async function verifyRecaptcha(token, expectedAction, ip) {
  ...
  try {
    ...
    if (expectedAction && data.action && data.action !== expectedAction) return { ok: false, reason: 'action mismatch (' + data.action + ')' };
    if (typeof data.score === 'number' && data.score < 0.5) return { ok: false, reason: 'low score ' + data.score };
