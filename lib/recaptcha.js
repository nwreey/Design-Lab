...
async function verifyRecaptcha(token, expectedAction, ip) {
  ...
  try {
    const resp = await fetch('https://www.google.com/recaptcha/api/siteverify', {
      ...
      // Cap is a sanity guard against absurd payloads only — real v3 tokens from
      // cloud-managed keys run ~2,200+ chars, and truncating a valid token makes Google
      // return invalid-input-response (exactly the bug this fixes; the old 2000 cap
      // silently cut them short).
      body: new URLSearchParams({ secret, response: token.slice(0, 10000), remoteip: ip || '' }).toString(),
    });
